...
LINT_LOAD_FILES = -l maint/android-lint.el
...
lint: $(LINT_FILES)
	$(RUN_EMACS) $(LINT_LOAD_FILES) -f android-lint-files $(LINT_FILES)

package-lint: $(EL_FILES)
	$(RUN_EMACS) $(LINT_LOAD_FILES) -f android-package-lint $(EL_FILES)

compile: $(EL_FILES)
	$(RUN_EMACS) -f batch-byte-compile $(EL_FILES)
